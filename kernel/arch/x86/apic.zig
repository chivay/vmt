...
fn getBase() mm.PhysicalAddress {
    ...
    const one: u64 = 1;
    const mask: u64 = ((one << @as(u6, @intCast(maxphyaddr))) - 1) - ((1 << 12) - 1);
    return mm.PhysicalAddress.new(((APIC_MSR.read() & mask)));
}

pub fn getLapicId() u8 {
    return @truncate((x86.cpuid(1, 0).ebx >> 24));
}
...
pub fn setTimerLVT(vector: u8, mask: Mask, mode: TimerMode) void {
    ...
    const value = vector | (@as(u32, @intFromEnum(mask)) << 16) | (@as(u32, @intFromEnum(mode)) << 17);
    reg.write(value);
}
